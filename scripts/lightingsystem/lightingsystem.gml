...
// specific targets.
#macro LIGHTSYSTEM_COMPATIBILITY_MODE false
    ...
    static draw = function() {
        if (surface_exists(_lightmap)) {
            ...
            if (_vivid) {
                ...
                draw_surface_ext(_lightmap, camera_get_view_x(cam), camera_get_view_y(cam), 1, 1, 0, c_white, 0.1);
            }
...
function LightSource(spr, attributes) constructor {
    ...
    static draw = function() {
        if (!_active) return;
        var cam = view_get_camera(view_current);
        draw_sprite_ext(_sprite, 0, _x - camera_get_view_x(cam), _y - camera_get_view_y(cam), _scale, _scale, 0, _color, _alpha + _flicker_val);
    }
